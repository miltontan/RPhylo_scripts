...
# This function will combine two time trees (phylo objects) into a single time tree (phylo object) if both trees are mutually exclusive clades. In other words, neither clade renders the other paraphyletic, and they will be joined in the resulting tree as sister clades.
# Combining these trees cannot use the bind.tip() function alone, because the root node of the MRCA of the two trees is not present in either tree. Thus, there is no node for binding.
...
# Generate a base tree to bind the two trees onto
